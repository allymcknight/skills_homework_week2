...
def find_common_items(list1, list2):
    ...
    for i in range(len(list1)):
        listdict[i] = list1[i]
        for elem in list2:
            if elem == listdict.get(i):
                common_nums.append(elem)
...
def encode(phrase):
    ...
    for i in range(len(phrase)):
        if phrase[i] == "e":
            phrase = phrase[:i] + "p" + phrase[i+1:]
        if phrase[i] == "a":
            phrase = phrase[:i] + "d" + phrase[i+1:]
        if phrase[i] == "t":
            phrase = phrase[:i] + "o" + phrase[i+1:]
        if phrase[i] == "i":
            phrase = phrase[:i] + "u" + phrase[i+1:]
...
def get_pirate_talk(phrase):
    ...
    for word in split_phrase:
        word = pirate_tranlations.get(word, word)
        pirate_phrase = pirate_phrase + " " + word
    pirate_phrase = pirate_phrase[1:]
